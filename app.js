...
let appState = {
    ...
    currentScreen: 'dashboard', // 'dashboard', 'addVisit', 'history', 'catalog', 'periodDetail'
    selectedPeriodId: null,
    pendingSync: [] // Визиты, ожидающие синхронизации
};
...
function renderInitialSetup() {
    ...
    content.innerHTML = `
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <input type="password" id="auth-token-input" placeholder="Токен" class="w-full p-2 border border-gray-300 rounded mb-3" value="${AUTH_TOKEN}">
            <button onclick="handleInitialize()" class="w-full bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600">Инициализировать</button>
        </div>
    `;
...
async function handleInitialize() {
    ...
    if (token) {
        // Обновляем AUTH_TOKEN в коде на лету (для тестирования)
        // В проде лучше хранить его в зашифрованном виде или просто в коде.
        // Здесь мы просто переопределяем константу в памяти.
        // Это упрощение для ввода токена на лету.
        Object.defineProperty(window, 'AUTH_TOKEN', { value: token, writable: false });
        try {
    ...
    content.innerHTML = `
        <div class="bg-white rounded-lg shadow p-4">
            ...
            <div class="mb-4">
                ...
                <input type="datetime-local" id="service-date" class="w-full p-2 border border-gray-300 rounded" value="${getCurrentDateTimeLocal()}">
            </div>
...
function addServiceRow(serviceData = {}) {
    ...
    const rowId = Date.now(); // Временный ID для строки
                ...
                ${periods.map(p => `
                    <div class="border rounded-lg p-3 cursor-pointer hover:bg-gray-50" onclick="showScreen('periodDetail', { periodId: '${p.period_id}' })">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm ${p.remaining_debt > 0 ? 'text-red-600 font-semibold' : 'text-green-600'}">${formatMoney(p.remaining_debt)}</span>
                        </div>
                        <div class="text-xs text-gray-500 mt-1">Услуг: ${p.services_count} | Выплат: ${p.payouts_count} | Статус: ${p.status}</div>
                    </div>
                `).join('')}
            </div>
        </div>
    ...
    content.innerHTML = `
        <div class="bg-white rounded-lg shadow p-4">
            ...
            ${remainingDebt > 0 && periodId !== 'CURRENT' ? `
                <button onclick="showPayoutModal('${periodId}', ${remainingDebt})" class="w-full bg-green-500 text-white py-2 rounded-lg hover:bg-green-600 mt-4">Внести оплату</button>
            ` : ''}
...
function showPayoutModal(periodId, remainingDebt) {
    ...
    modalContent.innerHTML = `
        ...
        <button onclick="handleFullPayout('${periodId}', ${remainingDebt})" class="w-full bg-green-500 text-white py-2 rounded-lg mb-2">Погасить полностью</button>
            ...
            <div class="space-y-2">
                ${servicesCatalog.map(s => `
                    <div class="flex justify-between items-center border-b py-2">
                        ...
                        <div class="flex space-x-2">
                            <button onclick="showCatalogEditModal('${s.service_id}', '${s.service_name}', ${s.base_price})" class="text-blue-500">✎</button>
                            <button onclick="handleDeleteService('${s.service_id}')" class="text-red-500">✕</button>
                        </div>
                    </div>
                `).join('')}
            </div>
            <button onclick="showCatalogEditModal()" class="w-full bg-blue-500 text-white py-2 rounded-lg mt-4">+ Добавить услугу</button>
        </div>
    `;
}
...
function getPeriodsSummary(transactions, payouts) {
    ...
    return Array.from(periodsMap.values()).sort((a, b) => {
        ...
        return b.period_id.localeCompare(a.period_id); // Сортировка по убыванию (новые сверху)
    });
}
...
async function initApp() {
    ...
    try {
        ...
        // Если не удалось загрузить данные, показываем экран настройки
